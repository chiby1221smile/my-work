...
function テキストを追加(スライド, テキスト, オプション) {
  ...
  // サイズを明示的に設定し直す（自動調整を防ぐ）
  テキストボックス.setWidth(ポイントをインチに変換(オプション.幅));
  テキストボックス.setHeight(ポイントをインチに変換(オプション.高さ));
  ...
  // すべての設定後に再度サイズを固定（自動調整を完全に防ぐ）
  テキストボックス.setWidth(ポイントをインチに変換(オプション.幅));
  テキストボックス.setHeight(ポイントをインチに変換(オプション.高さ));
  テキストボックス.setLeft(ポイントをインチに変換(オプション.左位置));
  テキストボックス.setTop(ポイントをインチに変換(オプション.上位置));
